...
class CaseReviewMailer < ApplicationMailer
  ...
  def confirm(candidate)
    @candidate = candidate
    mail(to: candidate, cci: 'revue-dossiers@beta.gouv.fr', subject: 'Revue de dossiers - Inscription confirmée')
  end

  def relance_j2(case_review)
    @case_review = case_review
    mail(to: case_review.candidate_email, cci: 'revue-dossiers@beta.gouv.fr', subject: 'Revue de dossiers - plus que 2 jours pour partager vos retours !')
  end

  def relance_j7(case_review)
    @case_review = case_review
    mail(to: case_review.candidate_email, cci: 'revue-dossiers@beta.gouv.fr', subject: 'Revue de dossiers - votre binôme attend votre retour !')
  end
  ...
  def send_feedbacks(case_review)
    @case_review = case_review
    mail(to: case_review.candidate_email, cci: 'revue-dossiers@beta.gouv.fr', subject: 'Revue de dossiers - donnez votre avis sur votre expérience !')
  end
end
