class PagesController < ApplicationController
  def home
    ...
    @conseils_articles = ConseilsArticle.order(created_at: :desc).first(3)
    @conseils_videos = ConseilsVideo.order(created_at: :desc).first(3)
    @webinaires = Webinaire.order(created_at: :desc).first(3)
