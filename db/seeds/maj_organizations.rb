...
require "csv"
filepath = File.join(Rails.root, 'db', 'organizations_data_2022.csv')
# Organization.destroy_all

i = 1

CSV.foreach(filepath, headers: :first_row) do |row|
  unless Organization.where("name ILIKE ?", "%#{row["name"]}%").empty?
    orga = Organization.where("name ILIKE ?", "%#{row["name"]}%").first
    orga.update(
